...
contract NFTXSimpleFeeDistributor is INFTXSimpleFeeDistributor, ReentrancyGuardUpgradeable, PausableUpgradeable {
	...
	function distribute(uint256 vaultId) external virtual override nonReentrant {
	...
	function initializeVaultReceivers(uint256 _vaultId) external override {
		...
		if (inventoryStaking != address(0)) INFTXInventoryStaking(inventoryStaking).deployXTokenForVault(_vaultId);
	}

	function changeReceiverAlloc(uint256 _receiverIdx, uint256 _allocPoint) public virtual override onlyOwner {
	...
	function removeReceiver(uint256 _receiverIdx) external virtual override onlyOwner {
