...
/// @author @0xKiwi_
/// @author @alexgausman
contract NFTXVaultFactoryUpgradeable is PausableUpgradeable, UpgradeableBeacon, INFTXVaultFactory {
	...
	function setFeeDistributor(address _feeDistributor) public virtual override onlyOwner {
		require(_feeDistributor != address(0));
		emit NewFeeDistributor(feeDistributor, _feeDistributor);
		feeDistributor = _feeDistributor;
	}

	function setZapContract(address _zapContract) public virtual override onlyOwner {
		emit NewZapContract(zapContract, _zapContract);
		zapContract = _zapContract;
	}

	function setFeeExclusion(address _excludedAddr, bool excluded) public virtual override onlyOwner {
	...
	function isLocked(uint256 lockId) external view virtual override returns (bool) {
	...
	function vault(uint256 vaultId) external view virtual override returns (address) {
		return vaults[vaultId];
	}

	function allVaults() external view virtual override returns (address[] memory) {
		return vaults;
	}
