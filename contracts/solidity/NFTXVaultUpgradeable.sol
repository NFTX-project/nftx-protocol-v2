	...
	INFTXVault
{
	...
	// Added in v1.0.3.
	function setVaultMetadata(string calldata name_, string calldata symbol_) external virtual override {
		onlyPrivileged();
	...
	// a similar interface.
	function deployEligibilityStorage(uint256 moduleIndex, bytes calldata initData) external virtual override returns (address) {
		onlyPrivileged();
		require(address(eligibilityStorage) == address(0), "NFTXVault: eligibility already set");
		INFTXEligibilityManager eligManager = INFTXEligibilityManager(vaultFactory.eligibilityManager());
		address _eligibility = eligManager.deployEligibility(moduleIndex, initData);
	...
	function redeem(uint256 amount, uint256[] calldata specificIds) external virtual override returns (uint256[] memory) {
	...
	function redeemTo(
		...
		uint256 totalFee = (_targetRedeemFee * specificIds.length) + (_randomRedeemFee * (amount - specificIds.length));
	...
	function swapTo(
		...
		require(count == specificIds.length || enableRandomSwap, "NFTXVault: Random swap disabled");
		...
		uint256 totalFee = (_targetSwapFee * specificIds.length) + (_randomSwapFee * (count - specificIds.length));
		_chargeAndDistributeFees(msg.sender, totalFee);
	...
	function allValidNFTs(uint256[] memory tokenIds) public view virtual override returns (bool) {
		if (allowAllItems) {
			return true;
		}
	...
	function nftIdAt(uint256 holdingsIndex) external view virtual override returns (uint256) {
		return holdings.at(holdingsIndex);
	}

	// Added in v1.0.3.
	function allHoldings() external view virtual override returns (uint256[] memory) {
	...
	function receiveNFTs(uint256[] memory tokenIds, uint256[] memory amounts) internal virtual returns (uint256) {
		require(allValidNFTs(tokenIds), "NFTXVault: not eligible");
		uint256 length = tokenIds.length;
		if (is1155) {
			// This is technically a check, so placing it before the effect.
			IERC1155Upgradeable(assetAddress).safeBatchTransferFrom(msg.sender, address(this), tokenIds, amounts, "");
	...
	function withdrawNFTsTo(
		...
		for (uint256 i; i < amount; ++i) {
			...
			uint256 tokenId = i < specificLength ? specificIds[i] : getRandomTokenIdFromVault();
	...
	function getRandomTokenIdFromVault() internal virtual returns (uint256) {
		uint256 randomIndex = uint256(
			keccak256(abi.encodePacked(blockhash(block.number - 1), randNonce, block.coinbase, block.difficulty, block.timestamp))
		) % holdings.length();
		++randNonce;
