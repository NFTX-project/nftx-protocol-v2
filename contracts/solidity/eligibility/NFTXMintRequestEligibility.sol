...
contract NFTXMintRequestEligibility is PausableUpgradeable, UniqueEligibility, NFTXEligibility, ERC721HolderUpgradeable, ERC1155HolderUpgradeable {
	...
	function __NFTXEligibility_init_bytes(bytes memory _configData) public virtual override initializer {
		(address _owner, address _vault, bool _negateElig, uint256[] memory _ids) = abi.decode(_configData, (address, address, bool, uint256[]));
	...
	function __NFTXEligibility_init(
		...
		IERC1155Upgradeable(_assetAddress).setApprovalForAll(address(vault), true);
	...
	function setEligibilityPreferences(bool _allowTrustedApprovals) external virtual onlyOwner {
		allowTrustedApprovals = _allowTrustedApprovals;
		emit AllowTrustedApprovalsSet(_allowTrustedApprovals);
	}

	function requestMint(uint256[] calldata tokenIds, uint256[] calldata amounts) external virtual {
		onlyOwnerIfPaused(0);
		...
		for (uint256 i; i < tokenIds.length; i++) {
			...
			require(mintRequests[msg.sender][tokenId] == 0, "No existing request");
			mintRequests[msg.sender][tokenId] = amount;
			if (_is1155) {
				require(amount > 0, "Must request with at least one");
				IERC1155Upgradeable(_assetAddress).safeTransferFrom(msg.sender, address(this), tokenId, amount, "");
			} else {
				require(amount == 1, "Must request with only one");
				IERC721(_assetAddress).safeTransferFrom(msg.sender, address(this), tokenId);
			}
		}
	...
	function claimUnminted(uint256[] calldata tokenIds, address[] calldata addresses) external virtual {
		require(tokenIds.length == addresses.length);
		INFTXVault _vault = vault;
		for (uint256 i; i < tokenIds.length; i++) {
			uint256 tokenId = tokenIds[i];
	...
	function reclaimRequestedMint(uint256[] calldata tokenIds) external virtual {
		...
		for (uint256 i; i < tokenIds.length; i++) {
			...
			if (_is1155) {
				IERC1155Upgradeable(_assetAddress).safeTransferFrom(address(this), msg.sender, tokenId, amount, "");
			} else {
				IERC721(_assetAddress).safeTransferFrom(address(this), msg.sender, tokenId);
			}
		}
	}

	function setUniqueEligibilities(uint256[] memory tokenIds, bool _isEligible) external virtual {
		if (!allowTrustedApprovals || !isGuardian[msg.sender]) {
			onlyPrivileged();
		} else {
			onlyOwnerIfPaused(0);
