...
contract NFTXRangeExtendedEligibility is OwnableUpgradeable, NFTXEligibility, UniqueEligibility {
	...
	function __NFTXEligibility_init_bytes(bytes memory _configData) public virtual override initializer {
		(address _owner, uint256 _rangeStart, uint256 _rangeEnd) = abi.decode(_configData, (address, uint256, uint256));
	...
	function setEligibilityPreferences(uint256 _rangeStart, uint256 _rangeEnd) external virtual onlyOwner {
		require(_rangeStart <= _rangeEnd, "Not valid");
		rangeStart = _rangeStart;
		rangeEnd = _rangeEnd;
		emit RangeSet(_rangeStart, _rangeEnd);
	...
	function setUniqueEligibilities(uint256[] calldata tokenIds, bool _isEligible) external virtual onlyOwner {
