...
async function main() {
	...
	const proxyController = await ProxyController.deploy(
		['NFTX Factory', 'Fee Distributor', 'LP Staking', 'StakingTokenProvider', 'Eligibility Manager', 'Inventory Staking'],
		[rinkebyVaultFactory, rinkebyFeeDistributor, rinkebyLPStaking, rinkebyStakingTokenProv, rinkebyEligManager, rinkebyInventoryStaking]
	);
	...
	const existingProxyController = await ethers.getContractAt('ProxyController', '0xD6f0Dd9400E89A7062a20f8E06bf8C083b184508');

	console.log('Chaging vault factory proxy admin...');
	await existingProxyController.changeProxyAdmin(0, rinkebyMultiProxyController, {
		gasLimit: '300000'
	});
	console.log('Chaging fee distributor proxy admin...');
	await existingProxyController.changeProxyAdmin(4, rinkebyMultiProxyController, {
		gasLimit: '300000'
	});
	console.log('Chaging LP staking proxy admin...');
	await existingProxyController.changeProxyAdmin(3, rinkebyMultiProxyController, {
		gasLimit: '300000'
	});
	console.log('Chaging staking token provider proxy admin...');
	await existingProxyController.changeProxyAdmin(2, rinkebyMultiProxyController, {
		gasLimit: '300000'
	});
	console.log('Chaging eligibility manager proxy admin...');
	await existingProxyController.changeProxyAdmin(1, rinkebyMultiProxyController, {
		gasLimit: '300000'
	});
	console.log('Chaging inventory staking proxy admin...');
	await upgrades.admin.changeProxyAdmin(rinkebyInventoryStaking, rinkebyMultiProxyController);
}
