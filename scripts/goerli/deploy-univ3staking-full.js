...
let uniV3FactoryAddr = "0x1F98431c8aD98523631AE4a59f267346ea31F984";
let uniNftManagerAddr = "0xC36442b4a4522E871399CD717aBDD847Ab11FE88";
...
async function main() {
  ...
  //   "0x1fDDE568C0f4e7b567068dA1264bc5fC58A60E86"
  // );
  ...
  const uniV3Staking = await upgrades.deployProxy(UniV3Staking, [uniV3FactoryAddr, uniNftManagerAddr, wethAddress, vaultFactory.address], {
    initializer: "__NFTXUniV3Staking_init",
    unsafeAllow: "delegatecall",
  });
  await uniV3Staking.deployed();
  console.log("UniV3Staking:", uniV3Staking.address);

  await feeDistrib.setV3StakingAddress(uniV3Staking.address);
  console.log("-- set univ3 staking address on fee distributer")
  ...
  await upgrades.admin.changeProxyAdmin(uniV3Staking.address, proxyController.address);
  console.log("-- updated proxy admin on univ3staking");
  ...
  console.log("-- set vault factory on unstaking zap");
  await unstakingZap.setInventoryStaking(inventoryStaking.address);
  console.log("-- set inventory staking on unstaking zap");
  await unstakingZap.setSushiRouterAndWeth(sushiRouterAddr)
  console.log("-- set sushi router and weth on unstaking zap")

  await vaultFactory.setFeeExclusion(unstakingZap, true);
  console.log("set fee exclusion on vault factory")
