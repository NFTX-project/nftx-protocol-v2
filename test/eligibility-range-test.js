...
describe('Eligibility Range', function () {
	before('Setup', async () => {
		...
		const RangeEligibility = await ethers.getContractFactory('NFTXRangeExtendedEligibility');
		rangeElig = await upgrades.deployProxy(RangeEligibility, [primary.address, 0, 0], {
	...
	it('Should disallow range to be set by user', async () => {
		await expectRevert(rangeElig.connect(alice).setEligibilityPreferences(0, 10));
	});
