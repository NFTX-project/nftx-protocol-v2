...
describe('Mainnet Inventory Staking Test', function () {
	...
	it('Should inventory stake with 721', async () => {
		vault = await ethers.getContractAt('NFTXVaultUpgradeable', '0x114f1388fab456c4ba31b1850b244eedcd024136');
		vaults.push(vault);
	...
	it('Should have locked balance', async () => {
		const xTokenAddr = await inventoryStaking.vaultXToken(31);
		const xToken = await ethers.getContractAt('XTokenUpgradeable', xTokenAddr);
		expect(await inventoryStaking.balanceOf(31, kiwi.getAddress())).to.equal(await xToken.balanceOf(kiwi.getAddress()));
	...
	it('Should mint to generate some rewards', async () => {
		const xTokenAddr = await inventoryStaking.vaultXToken(31);
		const oldBal = await vaults[0].balanceOf(xTokenAddr);
		await vaults[0].connect(kiwi).mint([7984], [1]);
		const newBal = await vaults[0].balanceOf(xTokenAddr);
	...
	it('Should add mint some for 1155', async () => {
		const oldShareValue = await inventoryStaking.xTokenShareValue(nft1155Id);
		await noPool1155NFT.connect(kiwi).setApprovalForAll(vaults[1].address, true);
		await vaults[1].connect(kiwi).mint([3], [4]);
		const newShareValue = await inventoryStaking.xTokenShareValue(nft1155Id);
