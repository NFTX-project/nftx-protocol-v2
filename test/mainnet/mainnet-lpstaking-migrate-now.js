...
describe('LP Staking Upgrade Migrate Now Test', function () {
	before('Setup', async () => {
		...
		liveBugUser = await ethers.provider.getSigner('0x8B0C8c18993a31F57e60d81761F532Ef14633153');
		liveZapLockUser = await ethers.provider.getSigner('0x08ceb8bba685ee708c9c4c65576837cbe19b9dea');
	...
	it('Should have locked balance', async () => {
		...
		const distToken = await ethers.getContractAt('IERC20Upgradeable', newDisttoken);
		const locked = await staking.lockedUntil(179, kiwi.getAddress());
		expect(await staking.lockedLPBalance(179, kiwi.getAddress())).to.equal(await distToken.balanceOf(kiwi.getAddress()));
