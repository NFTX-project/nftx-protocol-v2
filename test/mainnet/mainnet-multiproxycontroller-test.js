...
describe('MultiProxyController Mainnet Test', function () {
	...
	it('Should deploy multiproxycontroller', async () => {
		...
		multiProxyController = await MultiProxyController.deploy(
			['Vault Factory', 'Fee Distributor', 'LP Staking'],
			[nftx.address, feeDistrib.address, lpStaking.address]
		);
	...
	it('Should assign initial proxies properly', async () => {
		...
		const expectedInfo = ['0: Vault Factory', '1: Fee Distributor', '2: LP Staking'];
	...
	it('Should get all proxy addresses', async () => {
		...
		const expectedInfo = [nftx.address, feeDistrib.address, lpStaking.address];
	...
	it('Should allow adding proxies', async () => {
		const oldImpls = await multiProxyController.getAllImpls();
		await multiProxyController.addProxy('Inventory Staking', inventoryStaking.address);
		const newImpls = await multiProxyController.getAllImpls();
		expect(newImpls.length).to.equal(oldImpls.length + 1);
		const oldCopy = [...oldImpls];
	...
	it('Should upgrade the Fee Distributor', async () => {
		const NewFeeDistro = await ethers.getContractFactory('NFTXSimpleFeeDistributor');
		const feeDistro = await NewFeeDistro.deploy();
		await feeDistro.deployed();
	...
	it('Should upgrade the Inventory Staking', async () => {
		const NewStaking = await ethers.getContractFactory('NFTXInventoryStaking');
		const newStaking = await NewStaking.deploy();
		await newStaking.deployed();
	...
	it('Should pass over ownership of index 1', async () => {
		...
		const addr = await multiProxyController.getImpl(1);
		const proxyAdmin = await ethers.getContractAt('ProxyControllerSimple', '0x8e7488E4cEC0381e7Ac758234E1A8A793bE2fF30');
	...
	it('Should deploy multiproxycontroller', async () => {
		...
		multiProxyController2 = await MultiProxyController.deploy(
			['Vault Factory', 'LP Staking', 'Inventory Staking'],
			[nftx.address, lpStaking.address, inventoryStaking.address]
		);
	...
	it('Can read all impls', async () => {
		const impls = await multiProxyController2.getAllImpls();
		for (let i = 0; i < 4; i++) {
			expect(impls[i]).to.not.equal(zeroAddr);
		}
	});
