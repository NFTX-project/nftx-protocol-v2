...
describe('Main', function () {
	...
	it('Should not allow vault creation after pausing', async () => {
		const respPromise = nftx.connect(alice).createVault('CryptoPandas', 'PANDA', erc721.address, false, true);
	...
	it('Should properly return list of items', async () => {
		const list = await vaults[0].allHoldings();
	...
	it('Should allow redeeming one at a time by alice', async () => {
		...
		for (let i = 0; i < numLoops - 1; i++) {
			...
			expect(await erc721.balanceOf(vaults[0].address)).to.equal(numLoops * 2 - (i + 1));
			expect(await vaults[0].balanceOf(alice.address)).to.equal(BASE.mul(numLoops - (i + 1)));
		}
	});

	const firstSwapId = numLoops - 4;
	let swapId2;
	...
	it('[refresh NFT owner]', async () => {
		for (let i = 0; i < numLoops; i++) {
			...
			await erc721.connect(owner).transferFrom(owner.address, primary.address, i);
		}
	});

	it('Should let flash loan borrow and pay back', async () => {
		await flashBorrower.flashBorrow(vaults[0].address, vaults[0].address, BASE.mul(10000000000000));
		expect(await vaults[0].balanceOf(flashBorrower.address)).to.equal(0);
	});

	it('Shouldnt let flash loan steal funds', async () => {
		const tx = flashBorrower.flashBorrowAndSteal(vaults[0].address, vaults[0].address, BASE.mul(10000000000000));
		await expectRevert(tx);
		expect(await vaults[0].balanceOf(flashBorrower.address)).to.equal(0);
	});

	it('Shouldnt let flash loan reenter and steal funds', async () => {
		await flashBorrower.flashBorrowAndReenter(vaults[0].address, vaults[0].address, BASE.mul(10000000000000));
		expect(await vaults[0].balanceOf(flashBorrower.address)).to.equal(0);
	});
	...
	it('Should allow ERC1155 vault creation', async () => {
		const response = await nftx.createVault('CryptoNinjas', 'NINJA', erc1155.address, true, true);
		const receipt = await response.wait(0);
		const vaultId = receipt.events.find((elem) => elem.event === 'NewVault').args[0].toString();
		const vaultAddr = await nftx.vault(vaultId);
		const vaultArtifact = await artifacts.readArtifact('NFTXVaultUpgradeable');
	...
	it('Should set fees to 0', async () => {
		const perc25 = ethers.utils.parseEther('0.025');
	...
	it('Should allow ERC1155 with fee minting one at a time by alice', async () => {
		for (let i = 0; i < numLoops - 5; i++) {
			const tokenId = i;
			await erc1155.safeTransferFrom(primary.address, alice.address, tokenId, 1, []);
	...
	it('Should allow ERC1155 minting with fee in bulk by bob', async () => {
		...
		for (let i = 0; i < numLoops; i++) {
			const tokenId = numLoops + i;
			await erc1155.safeTransferFrom(primary.address, bob.address, tokenId, 1, []);
			tokenIds.push(tokenId);
		...
		expect(await vaults[1].balanceOf(bob.address)).to.equal(BASE.mul(numLoops).sub(PERC25_FEE.mul(numLoops)));
	});
	...
	it('Should allow ERC1155 target redeeming with fee one at a time by alice', async () => {
		for (let i = 0; i < numLoops - 2; i++) {
			...
			expect(await vaults[1].balanceOf(alice.address)).to.equal(BASE.mul(numLoops - (i + 1)).sub(PERC25_FEE.mul(i + numLoops + 1)));
		}
	});
	...
	it('Should allow upgrading the factory', async () => {
		const TestFactoryUpgrade = await ethers.getContractFactory('TestFactoryUpgrade');
		const testFactoryUpgrade = await TestFactoryUpgrade.deploy();
		await testFactoryUpgrade.deployed();
		const upgraded = await upgrades.upgradeProxy(nftx.address, TestFactoryUpgrade, {
			unsafeAllow: 'delegatecall'
		});
	...
	it('Should allow upgrading the vaults', async () => {
		const TestVaultUpgrade = await ethers.getContractFactory('TestVaultUpgrade');
		...
		const newVault = await ethers.getContractAt('TestVaultUpgrade', vaults[0].address);
