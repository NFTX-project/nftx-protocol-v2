...
describe('Main Eligibility', function () {
	before('Setup with Eligibility', async () => {
		...
		const MockFeeDistributor = await ethers.getContractFactory('MockDistributor');
		const mockFeeDistrib = await upgrades.deployProxy(MockFeeDistributor, [], {
		...
		nftx = await upgrades.deployProxy(Nftx, [vault.address, mockFeeDistrib.address], {
		...
		eligManager = await upgrades.deployProxy(Elig, [], {
	...
	it('Should allow vault creation', async () => {
		const response = await nftx.createVault('CryptoPandas', 'PANDA', erc721.address, false, false);
	...
	it('Should deploy a list module and add', async () => {
		...
		const module = await eligManager.modules(0);
	...
	it('Should allow deploying and assigning a List module', async () => {
		...
		const encoded = decoder.encode(['uint256[]'], [[0, 1, 2, 3, 4, 5]]);
